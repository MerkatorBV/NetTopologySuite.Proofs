(* ============================================================================
   nts-eval micro unit — claimId 423-b (RED)
   ...
   (`discrete_frechet_claim`) and deliberately NOT proved in this unit --
   no `Admitted`, no `Axiom`; the claim is a named `Definition ... : Prop`,
   so the Eval -> Qed matcher reports 423-b red.  The spec pins the
   computed value from both sides on nonempty lists:
     (lower bound) EVERY monotone coupling's max leash is >= dF(A,B);
     (attain)      SOME monotone coupling realises exactly dF(A,B).
   Together: dF is exactly the min-over-couplings max leash.
   Green target:
     Lemma discrete_frechet_min_coupling : discrete_frechet_claim.
   with the production home suggested as theories/FrechetDiscrete.v
   (structural induction on the nested recurrence; no sqrt anywhere),
   same WITNESS tag.
...
(* RED: no proof of the claim in this unit or in production.  Green must Qed
   `discrete_frechet_min_coupling` with this statement (micro-kernel) and a
   production mirror (suggested theories/FrechetDiscrete.v).  The Green is
   Meinert's Theorem-1-grade correctness content: the recurrence computes
   the two-frogs semantics. *)
