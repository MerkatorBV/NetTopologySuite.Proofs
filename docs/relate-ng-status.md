...
- **Obtuse-at-v certificate** — leftover `Ⅱ` classified.
  ...
  Fill stays `im_unsupported`. Completeness is mixed-cone
  (`RelateNGComplete.v : mixed_cone_pair_unsupported`). Do not
  bucket with leftover `Ⅰ`. Do not mint leftover `Ⅴ`.
