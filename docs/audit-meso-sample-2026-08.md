...
| # | Item | Notes / flags |
|---|---|---|
| B1 | PA-footer backfill: `PassesThrough_b64_grid_exact.v` (0/79) first, then `InCircle_b64_exact.v` (8/77), `HotPixel_b64.v` (45/97), and the 8 other `theories-flocq/` files with no footers | Audit-safe (files are exempted; `audit_axioms.sh` exempts before inspecting), compile-cost only (Flocq cone).  Idiom: 659146b "Sync: add in-file Print Assumptions audit lines". |
