...
- **Stack mismatch (porting cost)**: that project uses Coquelicot's real-analysis primitives (`RInt`, `is_derive`, `Derive`). Our corpus targets Rocq 9.2.0 + Flocq 4.2.2. The R-side identities would need re-proof in Flocq's native `Reals` framework (Coquelicot's `RInt` ↔ `RiemannInt`, `is_derive` ↔ Flocq's derivative predicate). Estimated ~3-5 days of mechanical translation: the tactic recipes (`auto_derive`, `Derive` rewrites, `ring`) are tactic-name preserved.
