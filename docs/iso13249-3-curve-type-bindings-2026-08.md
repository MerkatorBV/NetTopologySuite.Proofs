...
#630 — the pairwise contact kernel), with two fail-closed residues named in
the throws (degenerate closed segment; nearly-cocircular ambiguity band), so
...
compound/CurvePolygon simplicity and the IsValid wiring — the `615-h` lane,
continued at #634),
preferred over
