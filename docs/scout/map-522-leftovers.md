...
`Ⅶ` lens classified; `Ⅷ` inside classified; `Ⅸ` nest classified), #67 /
...
(classified; fill token). Completeness is an unnamed
swapped nest pair (not leftover `Ⅹ`). Do not
swap them. Do not mint leftover `Ⅹ`.
...
- Leftover ids switch to precomposed Roman numerals. `Ⅰ` = mutual
  ...
  (classified). Completeness is an unnamed same-side
  swapped nest pair (not leftover `Ⅹ`). `522-n` is not minted.
...
- **`Ⅶ`** is compiled (QED). Chart: [`map-lens-cert.md`](map-lens-cert.md).
  ...
  Ticket 32 closed. Epic #522 stop is QED ∨ QEX
...
unnamed ── swapped nest after leftover Ⅸ ── live completeness cex
