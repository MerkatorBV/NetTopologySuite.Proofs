...
| Completeness | **false** (unnamed swapped nest pair after leftover `Ⅸ`); stop is QED ∨ QEX | `RelateNGTouchNest.v : triangle_pair_regime_ccw_stop` |
...
Honest decline golden vector is an unnamed swapped nest pair
after leftover `Ⅸ`, not the #530 pair (that pair is classified disjoint).
...
- **Nest certificate** — leftover `Ⅸ` classified
  ...
  Fill stays `im_unsupported`. Completeness is an unnamed swapped
  nest pair (`RelateNGNestCex.v : unnamed_ccw_pair_unsupported`).
