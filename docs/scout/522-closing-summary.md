...
| Completeness | **false** (unnamed inside pair after leftover `Ⅶ`); stop is QED ∨ QEX | `RelateNGTouchLens.v : triangle_pair_regime_ccw_stop` |
...
Honest decline golden vector is an unnamed inside pair after leftover
`Ⅶ`, not the #530 pair (that pair is classified disjoint).
...
- **Lens certificate** — leftover `Ⅶ` classified
  ...
  Fill stays `im_unsupported`. Completeness is an unnamed inside pair
