(* ==========================================================================
   C3eEfCorridorAssumption.v

   [H-bridge attack, C-3e step A] ef-vs-corridor assumption + corridor_absorbs_ef
   probe.  Uniform corridor clearance thresholds (`corridor_safe_half` /
   `_third`) and inheritance lemmas: any straddle offset `ef` below half of a
   walk-dart delta0 inherits the corridor's ring-freedom at every height in
   the window.  Also the definitional identities tying straddle samples to
   corridor points.

   Pure real arithmetic; no `Admitted` / `Axiom` / `Parameter`; allowlist only.
