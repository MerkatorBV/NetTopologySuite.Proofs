(* ============================================================================
   ...
   **CompoundCurve joints only**. A CompoundCurve is a sequence of
   ...
   PhaseBRequiredGap while mixed LS–CS is host Decline — sidecar
   same-kind joint Hits are real math, not host Landed.
...
(* WITNESS: campaign=B rung=B.2 claim=0007
   ...
   land=Phase-B.2-letter-only *)

(* B.1 names, reused. Letter-local names; CompoundCurve required-type
   status stays Gap (mixed LS–CS still host Decline). *)
...
(* -------------------------------------------------------------------------- *)
(* Phase B.2 letter lands. CC required-type stays Gap (mixed Decline).       *)
(* CP / H⊥ / SQL/MM cathedral stay parked.                                   *)
(* -------------------------------------------------------------------------- *)
