(* ============================================================================
   ...
   Leftover `Ⅵ` / `Ⅶ` / `Ⅷ` / `Ⅸ` / `Ⅹ` classify same-cone / lens /
   inside / nest / swap. Completeness is still FALSE on an unnamed identical pair.
   ...
   FALSE (unnamed identical pair after leftover `Ⅹ`). Not an ADR-0004 remint.
...
(* 522-m: leftover Ⅹ classified the swap. Live cex is unnamed
   A=B=(0,0)(4,0)(0,4). Do not mint leftover `Ⅺ`. *)
...
(* WITNESS {"claimId":"522-j","topic":"relate","lemma":"triangle_pair_regime_ccw_incomplete","title":"Classifier completeness is still false after leftover Ⅹ: an unnamed identical CCW pair emits TPR_Unsupported","file":"theories/RelateNGComplete.v","witness":"522-j-sentinel-cex","board":"#577"} *)
Theorem triangle_pair_regime_ccw_incomplete :
  exists ax ay bx by_ cx cy dx dy ex ey fx fy : R,
    ...
    triangle_pair_regime ax ay bx by_ cx cy dx dy ex ey fx fy
      = TPR_Unsupported.
Proof.
  exists 0, 0, 4, 0, 0, 4, 0, 0, 4, 0, 0, 4.
  split; [unfold gdbl; lra|].
  split; [unfold gdbl; lra|].
  exact unnamed_ccw_pair_unsupported.
Qed.

(* WITNESS {"claimId":"522-m","topic":"relate","lemma":"triangle_pair_regime_ccw_incomplete_not_tjunction","title":"Filtered CCW-completeness is still false: an unnamed identical CCW pair declines after leftover Ⅹ classifies the swap nest","file":"theories/RelateNGComplete.v","witness":"522-m-complete-filtered","board":"#522"} *)
