(* ============================================================================
   ...
   (a) GENERIC MAX-MIN LAYER.  The aggregation facts of the 423-a Green
       ...
       upper bound) use no property of the score function at all, so
       they are proved once in a Section over an abstract
       score : Point -> Point -> R, then instantiated at the UNSQUARED
       metric dist (the triangle inequality lives there; the squared
       convention of 423-a's claim surface cannot carry it, since
       (a+b)^2 <> a^2 + b^2).
   ...
   on top -- no claim surface is redefined, and HausdorffDiscrete.v is
   untouched (a later refactor can re-base its dist_sq machinery on the
   generic section here once #431 merges).
...
From NTS.Proofs Require Import Distance HausdorffMetricSym.
Import ListNotations.
Open Scope R_scope.

(* -------------------------------------------------------------------------- *)
(* §1  Generic max-min layer: the aggregation facts are score-agnostic.       *)
(* -------------------------------------------------------------------------- *)
