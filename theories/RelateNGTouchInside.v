(* ============================================================================
   ...
   ----------------------------------------------------------------------------
   Leftover Ⅷ: name the leftover-Ⅶ nested-containment residue.

   Accept as leftover-Ⅷ inhabitance. Reject as a contains remint
   or a soundness theorem. Constructor is “escaped every prior arm,
   then A verts sit strictly interior to B,” not “this is
   contains / interiors meet.”
   `triangle_pair_regime_inside` is inhabitance, not soundness.
   `classify_triangle_pair`'s TPR_Inside arm is True — no
   denotation. There is no TPR_Inside ⇒ interiors meet.

   Compiled pair A = (1,1)(2,1)(1,2), B = (0,0)(4,0)(0,4). Both
   CCW. A sits strictly inside B. No shared vertex. `contains_b`
   is one-sided (B-in-A) and misses. `overlap_b` is still a
   vertex-stab certificate and misses. Detector `inside_b` is B
   CCW plus all three A vertices strictly interior to B — not a
   remint of `contains_b`. Constructor `TPR_Inside` stays on
...
(* WITNESS {"claimId":"Ⅷ","topic":"relate","lemma":"triangle_pair_regime_inside","title":"TPR_Inside inhabitance on the compiled leftover-Ⅷ nested-containment pair (not a contains denotation)","file":"theories/RelateNGTouchInside.v","witness":"Ⅷ-inside-cex","board":"leftover-Ⅷ"} *)
...
(* Misnamed: leftover Ⅰ / #570 / #572 / #567 still Disjoint /
   Overlap / TouchVertex / TouchEdge. Not a TPR_Inside claim. *)
