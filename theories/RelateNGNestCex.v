(* ============================================================================
   ...
   ----------------------------------------------------------------------------
   Leftover Ⅸ inhabitance (same-side shared-edge nest) plus the
   unnamed swap completeness cex after leftover Ⅸ (not leftover
   `Ⅹ`). Split out of RelateNGUnnamedCex.v so that file stays
   under the 1234-line split gate. UnnamedCex re-exports this
   module.

   Nest pair: A = (0,0)(4,0)(0,4), B = (0,0)(4,0)(1,1). Shared
   edge, same-side thirds; B's third vertex strictly inside A.
   Inhabits TPR_Nest. Detector `nest_b` is both CCW plus some
   shared edge plus some B vertex strictly interior to A — not
   a remint of `touch_edge_b` / `contains_b` / `overlap_b` /
   `inside_b`. Constructor is inhabitance, not TPR_Nest ⇒
   interiors meet. Do not remint leftover `Ⅷ`.

   Live cex: A = (0,0)(4,0)(1,1), B = (0,0)(4,0)(0,4). Shared
   edge, same-side thirds; `nest_b` is B-in-A only and misses.
   Do not mint leftover `Ⅹ`. Do not steal 522-j / 522-m.
...
(* WITNESS {"claimId":"Ⅸ","topic":"relate","lemma":"nest_pair_nest","title":"Leftover Ⅸ nest pair inhabits TPR_Nest (not a touch_edge/contains/overlap denotation)","file":"theories/RelateNGNestCex.v","witness":"Ⅸ-nest-cex","board":"leftover-Ⅸ"} *)

(* -------------------------------------------------------------------------- *)
(* Unnamed swap completeness cex after leftover Ⅸ. Not leftover `Ⅹ`.        *)
(* A = (0,0)(4,0)(1,1), B = (0,0)(4,0)(0,4). Shared edge; thirds same       *)
(* side; nest_b is B-in-A only and misses.                                   *)
(* -------------------------------------------------------------------------- *)
...
Proof.
  ...
  destruct (Rlt_dec 0 (gtri 0 0 4 0 1 1 (mkPoint 0 4))) as [H1 | _];
    [ exfalso; leftover_ix_gtri_contra H1 | ].
  rewrite !orb_false_r, andb_false_r.
  reflexivity.
Qed.
...
Proof.
  unfold triangle_pair_regime.
  rewrite swap_ccw_no_touch_edge.
  unfold contains_b.
