...
(*       proof requires multi-step geometric reasoning PLUS S4's              *)
(*       `arc_chord_intersect_sound`.  The IVT step below it is already Qed'd *)
(*       (ArcIntersectIVT); its arc-span promotion is Qed'd conditionally in  *)
(*       ArcChordSound.v.  The residual is the perpendicular-bisector geometry*)
(*       + the non-anchored-chord generalisation -- NOT IVT.                  *)
(*                                                                            *)
...
(* claimed "both gaps are independent of the IVT-blocked piece"; PR #50       *)
(* flagged (a) as IVT-gated.  UPDATE: the IVT step is now Qed'd               *)
(* (ArcIntersectIVT) and (a)'s arc-span soundness is Qed'd conditionally      *)
(* (ArcChordSound.v), so (a) is no longer IVT-gated -- its residual is the    *)
(* perpendicular-bisector geometry + the non-anchored-chord generalisation.   *)
(* (b) (the polygon-traversal lift) was always IVT-independent.  (c) is       *)
(* independent of both (a) and (b) and sits in Option A territory.            *)
(*                                                                            *)
...
(*                                                                            *)
(* RELATION TO `ArcHotPixelSagittaBridge.v` (do not conflate).  That file    *)
(* proves the OPPOSITE direction from what (a)/H_*_bridge need here: given   *)
(* an arc point Y (on-circumcircle, in-span, `arc_minor`), it exhibits a     *)
(* point of the CHORD SEGMENT within `sagitta` of Y.  (a) needs, for an      *)
(* arbitrary point of the chord-approximated POLYGON's interior, a nearby    *)
(* ARC point -- chord-region-to-arc-curve, not arc-curve-to-chord-segment.   *)
(* The two results share the sagitta vocabulary and the same `OM_perp_chord` *)
(* / `arc_radius_sq_pythagorean` toolkit, but neither implies the other:     *)
(* `ArcHotPixelSagittaBridge.v`'s bridge is a sound REJECTION filter (arc    *)
(* touches pixel ⟹ margin-dilated chord touches pixel) for the hot-pixel    *)
(* snap-rounding predicates of `HotPixel.v`, unrelated to the `boolean_op`   *)
(* / `to_geometry` overlay machinery this file targets.  It does not close  *)
(* (a), (b), or (c) above.                                                   *)
(* -------------------------------------------------------------------------- *)
