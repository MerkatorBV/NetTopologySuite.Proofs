...
| **#68** | Delaunay triangulation / Voronoi correctness | `Non-urgent` | `Triangle.v`, `Tin.v`, `GeneralTriangle{Parity,Separation}.v`, `RightTriangle*`; **track opened 2026-07-04**: `DelaunayEmptyCircle.v` (empty-circle predicate ↔ exact `b64_inCircle` sign, #364) + `DelaunayFlipWitness.v` (full lifted-determinant sign algebra incl. `inCircle_R_flip_witness_iff`, #379/#382). | Label now understates the state — predicate + flip algebra banked; geometric witness-sound flip is the active item |
...
| **TRI-DT** | Delaunay on (densified) curved boundaries | #68 | `theories-flocq/InCircle_b64_exact.v` (primitive), `DelaunayEmptyCircle.v`, `DelaunayFlipWitness.v`, `Triangle.v`, `Tin.v` | 🟡 in-circle primitive ✅ + empty-circle predicate ✅ + flip sign algebra ✅; geometric flip/insert correctness planned |
