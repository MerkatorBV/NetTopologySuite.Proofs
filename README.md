...
| 5 | Extraction toolchain + C# FFI to production NTS | **extraction toolchain shipped; in-process C ABI landed** — `Validate_binary64_extract.v` extracts 19 kernel functions with native-float overrides, and Phase 5 adds `libntsrocq` (`oracle/nts_ffi.{ml,h}` + `nts_ffi_stubs.c`): 18 of those functions callable in-process over a plain C ABI instead of forking `oracle_bin` per predicate. Gated by `oracle/gen_ffi_parity_tests.py` — ~1050 cases per run comparing the FFI against the oracle protocol as raw IEEE 754 bit patterns (NaN folded, `-0.0` distinguished), built + gated + published in CI. Still open: extracting an exact escalation path for `UNCERTAIN` (the proof, `b64_orient2d_exact_sound`, is Qed-closed; only the extraction is missing), non-Linux native assets, and the NTS call-site integration — see [`docs/phase5-ffi-abi.md`](docs/phase5-ffi-abi.md) | **binding published, not yet wired** — `oracle/csharp/RocqNative.cs` is the reference `DllImport` surface (enums, lifecycle, serialised calls); `NetTopologySuite.Robust.*` still reaches the kernel through the RocqRefRunner subprocess |
