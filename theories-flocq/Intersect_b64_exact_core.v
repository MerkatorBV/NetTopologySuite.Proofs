(* ============================================================================
   ...
   - First-stage exactness: the two outer orient2d evaluations (`qp0`,
     `qp1`) and the two coordinate differences (`dx`, `dy`) are bit-exact
     integer-valued binary64.
   - Scope B.1 denominator triple: the denominator subtraction is
     no-overflow safe (`b64_intersect_den_safe`), its B2R is the round of
     the exact cross-product difference (`b64_intersect_den_R_round`),
     and it is non-zero under the safety predicate
     (`b64_intersect_den_B2R_nonzero`).
