(* ============================================================================
   NetTopologySuite.Proofs.Flocq.PassesThrough_b64_grid_core
   ----------------------------------------------------------------------------
   SLICES 1-5: the on-grid reduction and the exactness layers.

   Slices 1-5 of the C1 grid-exactness ladder: on-grid definitions and the
   snap-consistency collapse (the REDUCTION: on the grid, passes-through
   grid-exactness is equivalent to single-touch grid-exactness); the integer
   grid IS the fixed-point grid; the division-free slab-guard layer; the
   coordinate-exactness layer; the max/min composition bridge.
